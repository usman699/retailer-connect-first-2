//
//  purchasereport.swift
//  retailer-connect-first
//
//  Created by Spark M1 on 08/05/2024.
//
...
class purchasereport:  UIViewController ,UIDocumentInteractionControllerDelegate, pagebutton {
    ...
    @IBAction func excelbutton(_ sender: Any) {
        ...
        // Header row
        var csvText = "#sr, purchase date ,supplier  name,  tax , totall bill, payment  status  \n"
        for (index, contact) in reportdata.enumerated() {
            // Rows
            let row = "#\( index), \(contact.date),  \(contact.supplierName),   \(contact.tax) ,\(contact.totalBill) ,\(contact.paymentStatus) ,   \n"
            csvText.append(row)
        }
    ...
    @IBAction func searchbutton(_ sender: Any) {
        ...
        let enddate = dateFormatter.string(from: calender.date)
        reportmodel.purchasesearchreportlist(paymentStatus: payment_status ?? "", start_date: startdate, end_date: enddate, customerid: custumerId, completion:
            {
            [self]
            result in
            switch result {
            case .success(let res ):
                print("decodind errro ")
                reportdata = []
    ...
    @IBAction func reset(_ sender: Any) {
        ...
        customers.selectedIndex = nil
        customers.text = nil
        paid.selectedIndex = nil
        paid.text = nil
   ...
   // @IBOutlet weak var nextpages: UIButton!
    ...
    var idArray = [Int]()
    var reportdata = [purchasemaindatas]()
  ...
  @IBOutlet weak var searchview: UIView!
    ...
    override func viewDidLoad() {
        ...
        collectionview.backgroundColor = .clear
           ...
           //     goingnextpage.setTitle("", for: .normal)
               // previouspage.setTitle("", for: .normal)
                ...
                tableview.register(UINib(nibName: "headerpurchasereport", bundle: nil), forCellReuseIdentifier: "hpurchasereportcell")
                let secondNib = UINib(nibName: "purchasereportcell", bundle: nil)
                tableview.backgroundColor = .white
                tableview.register(secondNib, forCellReuseIdentifier: "purchasereportcell")
    ...
    func fetchDatawithpage(page: Int) {
        reportmodel.purchasewithdata(page: page, completion: {result in
            ...
            case .success(let response):
                ...
                self.reportdata = response.data.data // Assuming saleReportItems is an array of SaleReportItem
                self.tableview.reloadData()
                self.currentpage = response.data.currentPage
                self.totalpages = response.totalPages
                    print("ttdsaf",self.totalpages)
...
//        goingnextpage.isEnabled = currentpage > 1
    ...
    func fetchpurchaseList(){
        ...
        reportmodel.puchasereportlist (completion: {
    ...
    */
extension purchasereport: UICollectionViewDelegate, UICollectionViewDataSource{
...
extension  purchasereport:  UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headerCell = tableView.dequeueReusableCell(withIdentifier: "hpurchasereportcell") as!  headerpurchasereport
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:purchasereportcell
        
        
        cell = tableView.dequeueReusableCell(withIdentifier: "purchasereportcell", for: indexPath) as!  purchasereportcell
        print("current", indexPath.item , (currentpage - 1) * perpage)
        let adjustedIndex = indexPath.item + 1  + (currentpage - 1) * perpage
        
        let currentCustomer =  reportdata[indexPath.row]
      
        cell.suppliersname.text = currentCustomer.supplierName
        ...
        cell.purchasedate.text = String(currentCustomer.date)
